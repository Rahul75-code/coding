...
select emp_id, salary, depart,
    row_number() over ( partition by depart, order by salary)  as row_num
from tbl1
)
...
Hive expects a proper date format like '2025-10-24', so you may need to convert 20251024 to that format first using string functions.
...
SELECT src.customer_id, src.name, src.address, CURRENT_DATE, NULL, 'Y'
FROM source_table src
...
(s.name <> t.name ors.city <> t.city);

insert into tgt(,,,,,)
select s,name,,,,,current_date(), null, 'y'
from stg as s 
left join tgt as t 
on t.id = s.id and t.flage ='y'
where t.id is null
...
--from sales table with column product_id, quantity, price, calculate the total revenue, 
--also handle null value,
--Filter revenue by date range, and give top 3 product id by total revenue
...
--📊 SQL Query to Get Revenue Trend Over Time (Monthly)
...
--✅ Databases That Support DATE_TRUNC()
Database		Support				notes
PostgreSQL		✅Yes				ative support
Snowflake		✅Yes				fully supported
Amazon Redshift	✅Yes				same syntax as PostgreSQL
MySQLUse		DATE_FORMAT() or 
				EXTRACT()			DATE_FORMAT(order_date, '%Y-%m-01')
SQL 			ServerUse FORMAT() 
				or DATEADD/DATEDIFF	DATEADD(month, DATEDIFF(month, 0, order_date), 0)
Oracle			Use TRUNC()			TRUNC(order_date, 'MM')

----------------------------------------------------------------------------------------

In SQL, unpivoting refers to the process of converting columns into rows — the opposite of pivoting, which turns rows into columns.
...
WITH ranked AS (
  SELECT
    Col_A,
    Col_B,
    ROW_NUMBER() OVER (PARTITION BY Col_A ORDER BY order_col) AS rn
...
window function 
how check the error in the data, how to filter data in sql

tbl booking list srt dt end dt cru membr, 
---------------------------------------------------------

---------------------------------------------------------

-- Passenger Manifest and Cabin Assingments
-- A cruise ship has various cabins (inside, oceanview, balcony, suite) and passeng